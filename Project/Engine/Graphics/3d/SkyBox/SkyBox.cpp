...
namespace MiiEngine {
	...
	void SkyBox::Draw(BlendMode mode) {
		...
		/// ===コマンドリストに設定=== ///
		// PSOの設定
		Service::Render::SetPSO(commandList, PipelineType::PrimitiveSkyBox, mode);
		// Viewの設定
		...
		// テクスチャの設定
		Service::Render::SetGraphicsRootDescriptorTable(commandList, 2, textureFilePath_);
		// 描画（Drawコール）
	...
	void SkyBox::EnvironmentMapDataWrite() {
		common_->SetEnvironmentMapData(environmentMapInfo_.isEnvironmentMap, environmentMapInfo_.strength);
	}
}