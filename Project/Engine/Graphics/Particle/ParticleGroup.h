...
namespace MiiEngine {
	...
	public:
		...
		void Initialize(const Vector3& translate, const ParticleDefinition& definition);
	...
	private:
		...
		struct Group {
			...
			Vector3 previousEmitterPosition = { 0.0f, 0.0f, 0.0f }; // 前回のエミッタ位置
			Vector3 previousEmitterRotation = { 0.0f, 0.0f, 0.0f }; // 前回のエミッタ回転
		};
