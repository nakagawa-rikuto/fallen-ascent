...
/// 土煙パーティクル
///=====================================================///
class DustCloudParticle : public ParticleGroup {
public:

    DustCloudParticle(); // コンストラクタでの処理は行わない
    ~DustCloudParticle();
...
private:
    // === コピー禁止 === ///
    DustCloudParticle(const DustCloudParticle&) = delete;
    DustCloudParticle& operator=(const DustCloudParticle&) = delete;

    // 土煙設定
    float spreadRadius_;   // 地面での広がり範囲
    float upwardSpeed_;    // 上昇速度
    float gravity_;        // 重力(負の値)
    float maxLifetime_;    // パーティクルの最大寿命
    bool hasEmitted_;      // パーティクルが発生したかのフラグ
